...
def render_add_card_tab():
    ...
    with tab1:
        ...
        card_type = st.selectbox(
            "Card Type",
            options=["flashcard", "multiple_choice", "multi_select", "true_false", "ponder"],
            format_func=lambda x: {
                ...
                "ponder": "💭 Ponder (open reflection, no single answer)"
            }[x],
        ...
        with st.form("add_card_form", clear_on_submit=True):
            ...
            # Ponder card fields
            if card_type == "ponder":
                st.subheader("Ponder Question")
                st.info("💭 Ponder cards have no single correct answer. Users reflect and can optionally share their thoughts.")
                seed_thought = st.text_area(
                    "Seed Thought / Prompt (optional)",
                    height=100,
                    help="A guiding thought or scripture shown below the question to help users reflect.",
                    key="ponder_seed"
                )
                card_data["answer"] = ""
                if seed_thought.strip():
                    card_data["seed_thought"] = seed_thought.strip()
            ...
            if submitted:
                ...
                elif card_type != "ponder" and not card_data.get("answer", "").strip():
